...
#aud = book.currencies(mnemonic='AUD')
#xbt = book.currencies(mnemonic='XBT')
#
## naive datetimes are treated as local, converted back to UTC for storage, and converted again to local on read
#p = piecash.core.commodity.Price(xbt, aud, datetime.now(), decimal.Decimal('2.777777'), 'last', 'from my script')
#
## aware datetime in UTC will be written without conversion (but still converted into local on read)
#dt = datetime(2015,6,1,0,0,0,0,timezone.utc)
#p = piecash.core.commodity.Price(xbt, aud, dt, decimal.Decimal('2.444444'), 'last', 'from my script')
#
#book.add(p)
#book.save()
#
## to get a date from local into UTC (as it was/will be stored), do
#book.prices[-1].date.astimezone(timezone.utc)
#
#################################################################################
#
#from urllib import request
...
## https://www.ecb.europa.eu/stats/exchange/eurofxref/html/index.en.html
#
#url = request.urlopen('https://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist-90d.xml') # last 90 days
#
#url = request.urlopen('https://www.ecb.europa.eu/stats/eurofxref/eurofxref-hist.xml') # since 1999
#
...
#
#for day in root.find('default:Cube', ns):
#    print(day.attrib['time'], day.find("./*[@currency='AUD']").attrib['rate'])
#
#
#################################################################################
#
## start_dt = datetime.datetime(2014,1,1,0,0,0,0,datetime.timezone.utc)
## today_dt = datetime.datetime.now(datetime.timezone.utc)
## days_since_start = (today_dt - start_dt).days
#
#start_d = datetime.date(2014,1,1)
#today_d = datetime.datetime.utcnow().date()
#days_since_start = (today_d - start_d).days
#
#{ start_d + datetime.timedelta(x) for x in range(days_since_start + 1) }
#
#[ start_d + datetime.timedelta(x) for x in range(days_since_start + 1) ]
#
#{ str(start_d + datetime.timedelta(x)) for x in range(days_since_start + 1) }
#
#[ str(start_d + datetime.timedelta(x)) for x in range(days_since_start + 1) ]
#
#4 not in {1,2,3,45}
...
#d[today] = 'hello'
#d
## {33: 'test', datetime.datetime(2015, 6, 27, 19, 44, 23, 632517, tzinfo=datetime.timezone.utc): 'hello'}
#
